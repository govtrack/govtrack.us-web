...
import datetime, lxml, os.path, re, json
...
bill_gpo_status_codes = {
    ...
    "eah": ("Passed the House with an Amendment", { BillStatus.pass_back_house }),
    "eas": ("Passed the Senate with an Amendment", { BillStatus.pass_back_senate }),
    "eh": ("Passed the House", { BillStatus.passed_simpleres, BillStatus.pass_over_house }),
    "ehr": ("Passed the House/Reprint", None),
    "eh_s": ("Passed the House/Star Print", None),
    "enr": ("Passed Congress", { BillStatus.passed_bill }),
    "renr": ("Passed Congress/Re-enrolled", None),
    "es": ("Passed the Senate", { BillStatus.passed_simpleres, BillStatus.pass_over_senate }),
    "esr": ("Passed the Senate/Reprint", None),
    "es_s": ("Passed the Senate/Star Print", None),
...
def get_bill_text_versions(bill):
    # Scan the bill's text-versions directory for downloaded bill text.
    from os import listdir
    d = bill.data_dir_path + "/text-versions"
    if not os.path.exists(d):
        return # don't yield anything, no text available
    for st in listdir(d):
        # statuscode/data.json are metadata files generated by the govinfo parser
        fn = bill.data_dir_path + "/text-versions/" + st + "/data.json"
        if os.path.exists(fn):
            yield st

        # also find docs.house.gov-scraped files
        m = re.match(r"(dhg-\d+).json$", st)
        if m:
            yield m.group(1)
...
def get_bill_text_metadata(bill, version):
    ...
    if version == None:
        ...
        dat = None
        for st in get_bill_text_versions(bill):
            d = get_bill_text_metadata(bill, st)
            if not dat or d["issued_on"] > dat["issued_on"]:
                dat = d
        return dat

    if version.startswith("dhg-"):
        return get_bill_text_metadata_dhg(bill, version)
    return get_bill_text_version_regular(bill, version)
...
def get_bill_text_metadata_dhg(bill, version):
    basename = bill.data_dir_path + "/text-versions"
    dat = json.load(open(basename + "/%s.json" % version))

    dat["version_code"] = version

    # human readable status name

    dat["status_name"] = "Preprint"
    if dat.get("consideration"): dat["status_name"] += " (" + dat["consideration"].title() + ")"
    dat["corresponding_status_codes"] = set()
    dat["source"] = "docs.house.gov"

    # parse date

    dat["issued_on"] = datetime.date(*(int(d) for d in dat["published_at"].split("T")[0].split("-")))
...
def load_bill_text(bill, version, plain_text=False, mods_only=False, with_citations=False):
    ...
    # this for historical bills that we don't have a MODS file for and docs.house.gov bills.
    else:
        gpo_url = dat["urls"].get("pdf")
        ...
        ret.update({
            ...
            "gpo_url": gpo_url,
            "gpo_pdf_url": dat["urls"].get("pdf"),
            "doc_version": dat["version_code"],
            "doc_version_name": dat["status_name"],
        })
